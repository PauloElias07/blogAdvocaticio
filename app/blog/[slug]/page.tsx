...
export default function PostPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <article className="mx-auto max-w-6xl px-6 py-20">
      ...
      {/* Cabeçalho fica acima das duas colunas, para ler sempre primeiro */}
      <div className="mt-6 max-w-3xl">
        <span className="eyebrow inline-block rounded-full border border-bronze px-3 py-1 text-[10px] uppercase text-goldLight">
          {post.category}
        </span>
      ...
      <div className={`mt-10 grid gap-10 ${post.video ? "lg:grid-cols-3" : ""}`}>
        {/* Sugestão de vídeo — sidebar sticky no desktop, aparece primeiro no mobile */}
        {post.video && (
        ...
        {/* Corpo do artigo */}
        <div className={`order-2 lg:order-1 max-w-3xl ${post.video ? "lg:col-span-2" : "lg:col-span-3"}`}>
          <div className="h-px w-full bg-panelLine" />
